...
import { OSSAnswers } from '../types';
import { OSS_QUESTIONS_FR } from '../constants';

interface OSSFormProps {
    initialAnswers: OSSAnswers;
    onNext: (answers: OSSAnswers) => void;
    onPrevious: () => void;
    onSkipToSummary: (answers: OSSAnswers) => void;
    onSave: (answers: OSSAnswers) => void;
}

const OSSForm: React.FC<OSSFormProps> = ({ initialAnswers, onNext, onPrevious, onSkipToSummary, onSave }) => {
    const [answers, setAnswers] = useState<OSSAnswers>(initialAnswers);
    ...
    return (
        <div className="space-y-8">
            <header className="text-center mb-4">
                <h2 className="text-xl font-bold text-gray-900">Score d'Oxford Épaule (OSS)</h2>
                <p className="mt-4 text-gray-600 max-w-2xl mx-auto">Veuillez répondre à ces questions concernant votre épaule. Cochez une seule case pour chaque question.</p>
            </header>

            <div className="space-y-8">
                {OSS_QUESTIONS_FR.map(section => (
                            ...
                            {section.options.map((option, index) => {
                                ...
                                return (
                                    <div key={index} className="flex items-center">
                                        ...
                                        />
                                        <label htmlFor={`oss-${section.id}-${index}`} className="ml-3 block text-sm text-gray-700 cursor-pointer">
                                            {option}
                                        </label>
                                    </div>
                                );
